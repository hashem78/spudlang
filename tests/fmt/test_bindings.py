...
class TestBinding:
    def test_simple_numeric(self):
        assert fmt().format_node(bind("x", num(1)), 0) == "x := 1"

    def test_string(self):
        assert fmt().format_node(bind("name", str_("spud")), 0) == "name := 'spud'"

    def test_boolean(self):
        assert fmt().format_node(bind("flag", bool_(True)), 0) == "flag := true"

    def test_binary_op(self):
        assert fmt().format_node(bind("x", binop(id("a"), "+", id("b"))), 0) == "x := a + b"

    def test_function_call(self):
        assert fmt().format_node(bind("x", call("foo", num(1))), 0) == "x := foo(1)"

    def test_no_walrus_spaces(self):
        cfg = FmtConfig(spaces_around_walrus=False)
        assert fmt(cfg).format_node(bind("x", num(1)), 0) == "x:=1"
