from pydantic import BaseModel

from spud.core.pipeline.pipeline_stage import PipelineStage
from spud.stage_five.stage_five_token import StageFiveToken
from spud.stage_seven.resolve_result import ResolveResult
from spud.stage_six.program import Program


class ResolvedProgram(BaseModel, PipelineStage, frozen=True):
    """Output of the full pipeline (stages 1-7).

    Carries tokens, the program with all errors (parse + resolve),
    and the resolve result with the environment tree.
    """

    tokens: list[StageFiveToken]
    program: Program
    resolve_result: ResolveResult
